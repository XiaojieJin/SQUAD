...
from utils import (
    ...
    calculate_quality_score, cal_scrublet
)
import glob
import shutil
import h5py
...
class Squad:
    ...
    @classmethod
    def from_cosmx(cls, fov_number, common_path, file_prefix):
        """从CSV文件创建Squad对象
    ...
    @classmethod
    def from_merscope(cls, fov_number=None, path=None):
    ...
    @classmethod
    def from_visiumhd(cls, path, dataset_id):
    ...
    def compute_all_metrics(self):
        ...
        try:
            ...
            if not "array_row" in self.sdata.table.obs.columns:
                ...
                cal_sgcc(self.sdata, polygon_file=self.metadata["polygon_file"]) #TBD
                
                logger.info("计算信噪比...")
                cal_signal_noise_ratio(self.sdata, polygon_file=self.metadata["polygon_file"]) #TBD
