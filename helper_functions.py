...
def pipeline_pca(self, n_prin_comps=50, random_state=0, svd_solver='arpack'):
    ...
    n_prin_comps_new = min(n_prin_comps, X_obs.shape[0], X_obs.shape[1])
    pca = PCA(n_components=(n_prin_comps_new - 1), random_state=random_state, svd_solver=svd_solver).fit(X_obs)
