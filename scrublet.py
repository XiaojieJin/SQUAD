...
class Scrublet():
    ...
    def scrub_doublets_2(self, synthetic_doublet_umi_subsampling=1.0, use_approx_neighbors=True, distance_metric='euclidean', get_doublet_neighbor_parents=False, min_counts=3, min_cells=3, min_gene_variability_pctl=85, log_transform=False, mean_center=True, normalize_variance=True, n_prin_comps=30, svd_solver='arpack', verbose=True):
        ''' Standard pipeline for preprocessing, doublet simulation, and doublet prediction
        ...
        #     print_optional('Embedding transcriptomes using Truncated SVD...', verbose)
        #     pipeline_truncated_svd(self, n_prin_comps=n_prin_comps, random_state=self.random_state, algorithm=svd_solver) 
        self.set_manifold(self._E_obs_norm, self._E_sim_norm)          

        print_optional('Calculating doublet scores...', verbose)
        self.calculate_doublet_scores_2(
            use_approx_neighbors=use_approx_neighbors,
            distance_metric=distance_metric,
            get_doublet_neighbor_parents=get_doublet_neighbor_parents
            )
    ...
    def calculate_doublet_scores_2(self, use_approx_neighbors=True, distance_metric='euclidean', get_doublet_neighbor_parents=False):
        ''' Calculate doublet scores for observed transcriptomes and simulated doublets
        ...
        self._nearest_neighbor_classifier_2(
            k=self.n_neighbors,
            exp_doub_rate=self.expected_doublet_rate,
            stdev_doub_rate=self.stdev_doublet_rate,
            use_approx_nn=use_approx_neighbors, 
    ...
    def _nearest_neighbor_classifier_2(self, k=40, use_approx_nn=True, distance_metric='euclidean', exp_doub_rate=0.1, stdev_doub_rate=0.03, get_neighbor_parents=False):
        manifold = scipy.sparse.vstack([self.manifold_obs_, self.manifold_sim_])
        doub_labels = np.concatenate((np.zeros(self.manifold_obs_.shape[0], dtype=int), 
                                      np.ones(self.manifold_sim_.shape[0], dtype=int)))
